...
// data = row, col, sensor_id, mux_id, mux_channel, sensor 68 is a dummy for encoder
const sensor_to_matrix_map_t sensor_to_matrix_map[] = {
    ...
    {4,0,59,0,11},{4,1,60,0,12},{4,2,61,1,9}, {4,3,62,1,13},{4,4,63,3,11},{4,5,64,3,12},{4,6,65,4,9}, {4,7,66,4,11},{4,8,67,4,12},{4,9,68,0,0}
};
...
static inline uint8_t rescale(uint16_t sensor_value, uint8_t sensor_id) {
...
void noise_floor_calibration(void) {
    ...
    // Calculate noise floor based on the 25th percentile
    for (uint8_t sensor_id = 0; sensor_id < SENSOR_COUNT; sensor_id++) {
...
void noise_ceiling_calibration(void) {
    ...
    for (uint8_t i = 0; i < SENSOR_COUNT; i++) {
        ...
        for (uint8_t j = 0; j < NOISE_CEILING_SAMPLE_COUNT; j++) {
            sum += he_readkey_raw(i);
        }

        average_value = sum / NOISE_CEILING_SAMPLE_COUNT;
...
int he_init(he_key_config_t he_key_configs[], size_t count) {
    ...
    // set runtime and eeprom from defaults on first boot
    if (eeprom_he_config.he_post_flash_flag == false) {
        eeprom_he_config.he_post_flash_flag = true;
...
static inline void select_mux(uint8_t sensor_id) {
    ...
    for (int j = 0; j < ARRAY_SIZE(mux_sel_pins); j++) {
        if (mux_channel & (1 << j)) {
...
bool he_update_key_rapid_trigger(matrix_row_t* current_matrix, uint8_t row, uint8_t col, uint8_t sensor_id, uint16_t sensor_value) {
    ...
    uint8_t hysteresis_margin = RT_HYSTERESIS;
...
bool he_update_key_keycancel(matrix_row_t* current_matrix, uint8_t row, uint8_t col, uint8_t sensor_id, uint16_t sensor_value) {
    ...
    if (currently_pressed && !previously_pressed) {
        if (++key_info->debounce_counter >= DEBOUNCE_THRESHOLD) {
            if (sensor_id == 32) a_pressed = true;
            if (sensor_id == 34) d_pressed = true;
        ...
        if (sensor_id == 32) a_pressed = false;
        if (sensor_id == 34) d_pressed = false;
...
static inline bool he_encoder_read(void) {
...
void add_sensor_sample(uint8_t sensor_id, uint16_t value) {
    sensor_data[sensor_id].samples[sensor_data[sensor_id].index % DEBUG_SAMPLE_COUNT] = value;
...
static double calculate_std_dev(uint8_t sensor_id) {
    double mean = 0.0;
    double std_dev = 0.0;
    ...
    mean /= DEBUG_SAMPLE_COUNT;
    for (int i = 0; i < DEBUG_SAMPLE_COUNT; i++) {
        std_dev += pow(sensor_data[sensor_id].samples[i] - mean, 2);
    }
    std_dev = sqrt(std_dev / DEBUG_SAMPLE_COUNT);
    return std_dev;
}

static double calculate_mean(uint8_t sensor_id) {
    double mean = 0.0;
    for (int i = 0; i < DEBUG_SAMPLE_COUNT; i++) {
        mean += sensor_data[sensor_id].samples[i];
    }
...
void he_matrix_print(void) {
    for (uint8_t i = 0; i < SENSOR_COUNT; i++) {
        char buffer[512]; 
        uint8_t sensor_id = sensor_to_matrix_map[i].sensor_id;
        uint8_t rescaled = rescale(he_readkey_raw(sensor_id), sensor_id);
        uint8_t row = sensor_to_matrix_map[i].row;
        uint8_t col = sensor_to_matrix_map[i].col;
...
void he_matrix_print_rapid_trigger_debug(void) {
    ...
    uint8_t rescale_readkey_raw = rescale(he_readkey_raw(i), i);
